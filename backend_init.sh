...
#
# The cache builder writes a `.populated` sentinel next to .venv only after
# pip-install succeeds. A crashed build still leaves the half-made .venv
# directory behind, so `-d` alone will happily copy in a venv with no pip
# and no site-packages. Require the sentinel, and require a real pip.
